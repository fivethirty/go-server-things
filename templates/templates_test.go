...
import (
	...
	"github.com/fivethirty/go-server-things/templates"
...
func TestNew(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:         "should error with both dir and fs",
			templatesDir: "/foo",
			templatesFS:  templatesFS,
			expectError:  true,
		},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			t.Parallel()
			_, err := templates.New(templates.Config{
...
func TestEmbeddedExecute(t *testing.T) {
	...
	tmpls, err := templates.New(templates.Config{
		TemplatesFS: subFS,
...
func testExecute(t *testing.T, tmpls *templates.Templates) {
	t.Helper()
	tests := []struct {
		name     string
		glob     string
